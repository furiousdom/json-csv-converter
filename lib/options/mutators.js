...
function validateRenamers(mutators, separator) {
  ...
  return validation(mutators, separator, 'oldPath', 'newPath', prompter);
}

function validateSetters(mutators, separator) {
  const prompter = prop => `Enter value to set "${prop}" property to? `;
  return validation(mutators, separator, 'propPath', 'value', prompter);
}

function validation(mutators, separator, firstPropName, secondPropName, prompter) {
